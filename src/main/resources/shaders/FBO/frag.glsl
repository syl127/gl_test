...
vec4 _bicubicX(sampler2D sampler, float xOff, vec4 xcub, vec2 uv) {
  return mat4(
...
vec4 textureBicubicFromNearest(sampler2D sampler, vec2 texCoord, vec2 texResolution) {
  ...
  vec4 xcub = cubic(xy.x);
  return mat4(
    _bicubicX(sampler, invRes.x, xcub, vec2(texCoord.x, texCoord.y - invRes.y)),
    _bicubicX(sampler, invRes.x, xcub, texCoord),
    _bicubicX(sampler, invRes.x, xcub, vec2(texCoord.x, texCoord.y + invRes.y)),
    _bicubicX(sampler, invRes.x, xcub, vec2(texCoord.x, texCoord.y + 2.0 * invRes.y))
  ) * cubic(xy.y);
}
...
void main() {
    ...
    switch(scalingType) {
        ...
        case 3: {
            texSample = textureBicubicFromNearest(tex, texCoord, imgSize);
            break;
        }
